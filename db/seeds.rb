ADMIN_EMAIL = 'admin@demo.exchange'
ADMIN_EMAIL_2 = 'admin@dinastyoffreedom.com'
...
admin_identity = Identity.find_or_create_by(email: ADMIN_EMAIL_2)
admin_identity.password = admin_identity.password_confirmation = ADMIN_PASSWORD
admin_identity.is_active = true
admin_identity.save!

admin_member = Member.find_or_create_by(email: ADMIN_EMAIL_2)
admin_member.activated = true
admin_member.authentications.build(provider: 'identity', uid: admin_identity.id)
admin_member.save!
...
# if Rails.env == 'development'
  ...
  foo = Identity.create(email: 'foo@demo.exchange', password: NORMAL_PASSWORD, password_confirmation: NORMAL_PASSWORD, is_active: true)
  foo_member = Member.create(email: foo.email, activated: true)
  foo_member.authentications.build(provider: 'identity', uid: foo.id)
  foo_member.tag_list.add 'vip'
  foo_member.tag_list.add 'hero'
  foo_member.save

  bar = Identity.create(email: 'bar@demo.exchange', password: NORMAL_PASSWORD, password_confirmation: NORMAL_PASSWORD, is_active: true)
  bar_member = Member.create(email: bar.email, activated: true)
